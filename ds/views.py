...
from ds.models import News, Gongzihao
...
def edituser(request, id):
    ...
    if request.method == "POST":
        err = ''
        us = User.objects.filter(username=request.POST.get('username', '')).exclude(pk=id)
        if us:
            err = u'重复的证件账号'
        us = User.objects.filter(last_name=request.POST.get('last_name', '')).exclude(pk=id)
        if us:
            err = u'重复的身份证'
        us = User.objects.filter(username=request.POST.get('last_name', '')).exclude(pk=id)
        if us:
            err = u'重复的证件账号'
        us = User.objects.filter(last_name=request.POST.get('username', '')).exclude(pk=id)
        if us:
            err = u'重复的身份证'
